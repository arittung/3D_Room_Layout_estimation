...
def conv3x3(in_planes, out_planes, stride=1):
    # in_planes : 입력 필터개수, out_planes : 출력 필터개수
    # padding: 커널 원소간의 거리, 늘릴수록 같은 파라미터수로 더 넓은 범위를 파악할 수 있게 됨. 기본값 1.
...
#def conv1x1(in_planes, out_planes, stride=1):
 ...
 #   return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, padding=(1, 0), bias=False)


class BasicBlock(nn.Module): # Residual Block의 전체코드
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        # forward에 필요한 layer 정의
    ...
    def forward(self, x):
        ...
        # downsampling이 필요한 경우 downsample layer를 block에 인자로 넣어주어야 함
        # downsample layer은 resnet class에서 정의하여 넣어줌.
        if self.downsample is not None:
            residual = self.downsample(x)

        # out과 residual(입력 텐서) 더한 후 relu 거침/
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1): # residual block을 생성
        # block: BasicBlock 구조사용, planes: input shape, blocks: layer 반복해서 쌓는 개수, stride: 고정
        # block : 기본 block 형태, blocks : block이 몇 개 쌓여있는가
        downsample = None

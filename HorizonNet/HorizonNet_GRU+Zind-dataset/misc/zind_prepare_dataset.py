...
def run(scene_id, split, args):
    paths = sorted(glob.glob(args.indir))

    if len(paths) == 0:
        print('no images found')
    # label = json.load(open(os.path.join(args.indir, scene_id, 'zind_data.json')))
    indir = args.indir[:-6]
    #print(type(args.indir))
    label = json.load(open(os.path.join(indir,'zind_data.json')))
    ...
    #print("--> ", len(paths))
    #print(type(label))
    #print(type(label_iterator(label)))
    #print("----->", len(label_iterator(label)))
    ...
    for i_path in paths:
        for floor_id, partial_room_id, pano_id, pano_data in label_iterator(label):
            ...
            basename = os.path.splitext(os.path.basename(i_path))[0]
            img_path = os.path.join(indir,  '%s.png' % basename)
            #print("-------------", img_path)
            ...
            img.save(os.path.join(args.outdir, split, 'img', '%s.png' % basename), quality=95, optimize=True)
            with open(os.path.join(args.outdir, split, 'label_cor', '%s.txt' % basename), 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--indir', required=True, default='data/')
